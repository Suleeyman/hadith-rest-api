...
class EditionService:
    ...
    def get_editions(
        self, languages: list[str], available_language: str | None
    ) -> list[Edition]:
        return list(self.repository.find_all(languages, available_language))

    # ----- WITH LOOKUP -----
    def get_edition_by_slug_join_books(
        self, slug: str, languages: list[str]
    ) -> EditionWithBooks:
        edition = self.repository.find_one_by_slug_join_books(slug, languages)
        if edition is None:
            raise EditionNotFoundError(slug)
